...
class FineTune(object):
    ...
    def _test2(self, test_loader):
        from .models.gcn_finetune import GCN
        model = GCN(self.config['dataset']['task'], **self.config["model"]).to(self.device)

        checkpoints_folder = os.path.join('MolCLR/ckpt', self.config['fine_tune_from'], 'checkpoints')
        state_dict = torch.load(os.path.join(checkpoints_folder, 'model.pth'), map_location=self.device)
        model.load_my_state_dict(state_dict)
        device = self.device
...
def prepare(test_loader=None):
    def _main(config, test_loader=None):
        dataset = MolTestDatasetWrapper(config['batch_size'], **config['dataset'])

        fine_tune = FineTune(dataset, config)
        fine_tune._test2(MolTestDataset('bace_train.csv', 'docking_score', 'regression'))
        return fine_tune
    ...
    for target in target_list:
        config['dataset']['target'] = target
        return _main(config, test_loader)
